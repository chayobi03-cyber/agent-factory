...
| `OPEN_DECISIONS_2026-08-25.md` | **CANONICAL** | The live decision register: every item that needs a person, with what was verified and what it costs to be wrong. Read this before planning a session; retire each entry as it is decided. D-01 through D-10 were resolved 2026-08-25 and are retained there as the record of why; **D-11 and D-12 are open** — near-miss abstention is not decidable by any lexical statistic, and the kernel has no model dependency while three RE_POC requirements need one. Both were raised by the M1 work itself. Add new decisions to this register rather than starting a fresh one. |
| `ARCHITECTURE_REFACTOR_PLAN_2026-08-19.md` (verification step) | **CANONICAL** | Its implementation sequence puts *Evidence + Claim verification* after hybrid retrieval; that step landed 2026-08-25 as `src/claim_verification.py`, a kernel module with no domain knowledge, consumed by `CERGateRuntime.evaluate`. Goal 1 ("verification in the shared kernel") and goal 2 ("domain behavior behind Domain Packs") are both satisfied: the mechanism is generic, the threshold comes from `domains/re/domain_pack.yaml`. |
| `../ADDING_A_DOMAIN.md` | **CANONICAL** | How to stand up a new engineering domain: a `domain_pack.yaml` plus a folder of documents, no Python. Written 2026-08-26 when the generic engine was extracted from the RE pack and `domains/thermal/` was added as a code-free worked example. States the two measured limits (D-11, D-12) a newcomer would otherwise discover the hard way. |
