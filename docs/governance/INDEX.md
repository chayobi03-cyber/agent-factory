...
**Purpose:** so a new session can tell in under a minute which of the 23 documents in `docs/governance/` are the live, actively-enforced contracts versus historical record. Nothing below was deleted — this is a status index, not a cleanup.
...
| `OPEN_DECISIONS_2026-08-25.md` | **CANONICAL** | The live decision register: every item that needs a person, with what was verified and what it costs to be wrong. `D-01` is the one blocking M1 expansion. Read this before planning a session; retire each entry as it is decided. |
