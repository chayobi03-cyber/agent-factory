...
| 3.5 | `asset.schema.yaml` defines the asset record | It is a YAML sketch (`id: string`). No `$schema`, no `type`, no `required`, no validator, no consumer. It cannot reject the six non-conformant files above — which is why it did not. Contrast `schemas/execution_evidence.schema.json` in this repo: real Draft 2020-12, 13 required fields, validated in CI. |
